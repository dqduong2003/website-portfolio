  ...
  return (
    <>
      <motion.div
        ...
        <div
          className="absolute bottom-0 left-0 p-6 sm:p-7 w-full translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
          <motion.div
            layoutId={`subtitle-${layoutId}`}
            className="flex flex-wrap gap-1.5 mb-2.5">{subtitle}</motion.div>
          <motion.h3
            layoutId={`title-${layoutId}`}
            className="text-lg sm:text-xl font-semibold tracking-tight text-white">{title}</motion.h3>
        </div>
      </motion.div>
      ...
      <AnimatePresence>
        {isOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              layoutId={layoutId}
              className="relative w-full max-w-2xl max-h-[88vh] bg-card rounded-2xl overflow-hidden border border-border z-10 flex flex-col shadow-xl overflow-y-auto">
              <button
                onClick={() => setIsOpen(false)}
              ...
              <div className="p-6 sm:p-8 w-full flex flex-col">
                {tags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {tags.map((t) => (
                      <span
                        key={t}
                        className="inline-flex items-center rounded-sm bg-secondary border border-border px-2.5 py-1 text-xs text-secondary-foreground"
                      >
                        {t}
                      </span>
                    ))}
                ...
                <motion.h3
                  layoutId={`title-${layoutId}`}
                  className="font-serif text-2xl sm:text-3xl font-normal tracking-tight text-foreground mb-4">{title}</motion.h3>
